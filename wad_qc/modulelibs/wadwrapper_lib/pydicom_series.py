...
#
# Changelog:
# 20190506: Fix for 'FileDataset' object has no attribute 'ImagePositionPatient'; Fix for pydicom 1.2.x
# 20161025: added option to override default series splitting splitOnPosition
# 20161007: prepare for pydicom release 1
# 20160421: brought back to core functionality: removed main, get_data, show_data
...
def _splitSerieIfRequired(serie, series):
    ...
    for index in range(1, len(L)):
        ...
        # Get current slice
        ds2 = L[index]
        if not "ImagePositionPatient" in ds2:
            continue
...
def _getPixelDataFromDataset(ds):
    ...
    # AS 1.2 el = dict.__getitem__(ds, pixelDataTag)
    el = ds[pixelDataTag]
    ...
    # AS 1.2 dict.__setitem__(ds, pixelDataTag, el)
    ds[pixelDataTag] = el
    del ds._pixel_array
...
def read_files(path, showProgress=False, readPixelData=False, skipNonImageFiles=True, splitOnPosition=True):
    ...
    for filename in files:
        ...
        try:
            ...
            try:
                dcm = dicom.read_file(filename, deferSize, force=True)  # AS: some files still valid
            except:
        ...
        if skipNonImageFiles:
            try:
                sopclass = dcm.SOPClassUID
    ...
    def _finish(self):
        ...
        for index in range(len(L)):
            ...
            try:
                ...
                distance_sum += abs(ds1.SliceThickness)
